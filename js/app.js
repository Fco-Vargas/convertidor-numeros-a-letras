class Converter {
    ...
    addListener() {
        let el = document.getElementById('field-number');
        el.addEventListener('keyup', () => {
            if (el.value !== '') {
                this.converToText(el.value);
            } else {
    ...
    getTens(number) {
        ...
        let unit = number.charAt(1);
        if (number < 17) {
            return this.tenToSixteen[numberInt - 10];
        }
        if (number < 20) {
            return 'dieci' + this.getUnits(unit);
        }
        // Nombres especiales
        switch (number) {
            case '20':
        ...
        if (number < 30) {
            return 'veinti' + this.getUnits(unit);
        }
        let name = this.tens[number.charAt(0) - 3];
        if (unit > 0) {
            name += ' y ' + this.getUnits(unit);
        }
        return name;
    }
    ...
    getHundreds(number) {
        ...
        let hundred = number.charAt(0);
        // Obtener las decenas y unidades
        let ten = number.substr(1);
        if (number == 100) {
            return 'cien';
        }
        // Nombres especiales
        ...
        if (name === '') {
            name = this.getUnits(hundred) + 'cientos';
        }
        if (ten > 0) {
            name += ' ' + this.getName(ten);
        }
        return name;
    }
    ...
    getThousands(number) {
        ...
        // Obtener los centenas,decenas y unidades
